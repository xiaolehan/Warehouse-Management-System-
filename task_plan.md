...
**当前状态：** ✅ 阶段 1–3.7、5–8（六项需求改进全部落地）、阶段 9–13（生产研发部产线：部门角色/BOM/生产任务单+齐套预警/质检/生产端迁移，commit 490a489）、阶段 14–17（D60–D63 增量完善）、阶段 18–21 全部完成；会话 27 三项体验优化（commit e395161）；会话 28 defer 清理全清零（13 项 Minor + 5 项架构债，4 commit 58d79d3/30a710a/fa17767/9add31d，2026-09-11）；✅ 阶段 22 超管审计强化全部落地（ADR-0009，2026-09-13 会话 31，B1–B7/F1–F4 全绿，225 单测）；会话 32 阶段 22 手测闭环 + 侧边栏菜单滚动/手风琴（2026-09-13）；✅ 阶段 23 库存盘点模块全部闭环（ADR-0010，D81–D85，255 单测全绿 + curl E2E + 浏览器手测，2026-09-14 会话 33）；✅ 阶段 24 补料再补解锁+部分到货仍缺料通知（ADR-0011，D86–D87，260 单测全绿 + curl E2E 原死锁场景全链路，2026-09-14 会话 33）；会话 36 供应商联系人搜索命中展示优化（D89，纯前端，2026-09-15）；✅ 会话 37 前端业务错误提示单通道根治问题 #1 静默吞错（ADR-0012/D90，拦截器统一提示 + 39 文件清扫 172 死检查/~130 重复 toast，2026-09-15）；✅ 会话 38 优化批三项（D91 供应商筛选 filterable×2 / D92 首页预警卡片可点直达+预警中心读权限放开四部门成员 / D93 员工菜单手术，前端 build + 后端 compile + curl E2E 10/10，2026-09-16，待浏览器手测）；✅ 会话 38 作废解惑包（D94 保留作废语义+六页 VoidConfirmDialog 说明弹窗+审批流四页行内「作废审批中」+新端点 pending-void-biz-ids，curl E2E 14/14，2026-09-16，待浏览器手测）；✅ 会话 38 删除/作废口径统一+死锁解锁（D95 当天已生效单可提交作废审批/销售 admin 已出库单收口/前端按钮与后端同口径 + D96 仓储退货确认页源单下拉按角色门控，curl E2E 9+8 全绿，2026-09-16，待浏览器手测）；✅ 会话 38 作废态可视+主流程冻结（D97 四视图状态列已作废/已冲抵终态直显 + D98 作废审批中冻结全部 6 个确认方法——前端禁用+隐藏、后端 ensureNoPendingVoidApproval 守卫+ensureNormalStatus 补齐，curl E2E 15/15，2026-09-16，待浏览器手测）；✅ 会话 39 红冲全面停用+措辞通俗化（D99 界面「红冲」→「冲抵」+悬浮说明、void_red 提交与 createRedFlush 直废两路径焊死、CONTEXT 词条改写停用，curl E2E 8/8+内联回归，2026-09-19，待浏览器手测）；✅ 会话 40 用户手测三问题修复（D100 缺省供应商锚点复活+禁删守卫+种子行 / D101 直接进货回写进价+作废重算 / D102 物料进价历史弹窗+新端点 / D103 作废审批站内信化+移除右下角浮卡，后端 compile+前端 build 过，curl E2E T1/T3/T4/T5/T6 全绿，2026-09-19，待浏览器手测）；✅ 会话 41 四类跨部门单据流程时间线（D104 进货/采购申请/进货退货/销售退货时间线：DocumentTimelineService 四方法+四端点+通用 DocumentTimeline 组件+SalesTimeline 迁移，E2E 四链全绿含状态推进/驳回/作废审批中/voided/权限负测，2026-09-19，待浏览器手测）；✅ 会话 42 批次一四项（D105 齐套领料按单事实显示 / D106 销售日期=开单时间自动生成删日期选择器 / D107 成品入库两段式确认——生产提交申请不加库存+仓储确认才加库存转完成+撤销/驳回 / D108 初版误读为审批入口常驻，2026-09-20 经 /code-review 15 findings 全修（269 单测+33 项状态机 E2E），commit 76a3bd0；2026-09-20 用户纠正 D108 真意为「系统参数」页阈值设置项常驻，已回退误读入口改动、改为阈值行启动自愈（275 单测+9 项 E2E）；✅ 会话 44 批次二 D109 未知物料供应商匹配（ADR-0014：到货备注斜杠约定+仓储人工触发精确回写+条件更新防并发+来源单号审计，288 单测/E2E 28/28/code-review 7 findings 全修，2026-09-20）；✅ 会话 45 批次三 D110 一客户多型号（ADR-0013：销售单头行结构 biz_sales_detail 一单 N 成品行/同成品一单只一行，整单一次确认出库不拆单、退货按来源行退+行级可退量、价格偏离整单一笔审批、缺货按单汇总一条消息、生产联动锚定明细行、履约时间线按行展示、统计 SQL 下沉 detail JOIN、红冲死代码移除；300 单测 + E2E 27/27 + /code-review 15 findings 修 14 项（明细行级联软删/删除按钮权限/时间线发货时间戳/确认收尾与终止条件更新/唯一键兜底/缺货列门控/单价预填校验/ precision=0/批量进价与时间线查询/口径去重），2026-09-20）。三批次全部完成，待用户统一手测）
...
### 阶段 4：库存治理与追溯（三子项已全落定）
- 盘点 ✅ 已由阶段 23 落地（ADR-0010）；余料管理 🅿️ 2026-09-14 挂起（会话 33 grilling 两轮：余料=生产损坏料非 BOM 剩余，Round 2 四决策已定，恢复续 Round 3，详见 progress 会话 33 末条）；成品追溯 ⏸️ 2026-09-14 搁置（用户拍板：成品无非卖出或退回，无其他流向，追溯无对象）
...
- 已知问题（本次不修）：补料单按行部分到货→确认入库终态→若仍缺料，受"已入库补料单阻止再补料"既有规则（D59）约束，该生产单无法再补——待单独立项。**✅ 已由阶段 24 解决（2026-09-14，D86 守卫收窄在途状态 + D87 在途提示/仍缺料通知，ADR-0011）。**
...
### 阶段 22：超管审计强化——日志可读性 + 真只读 + 删除/导出（D76–D80 / ADR-0009，2026-09-13 会话 31 grilling 三轮定案）✅ 完成

- 背景（公司三问）：① 超管现在能看到什么（事实：前端白名单仅系统页面，业务页面全不可达，后端读接口基本放行；财务两模块显式排除）；② 操作日志目标类型/目标ID/请求路径看不懂（根因：targetId 从方法参数猜、before_data/after_data 从未赋值、requestUri 原始直出、前端零翻译）；③ 公司要超管看清每个人的操作 + 日志下载 + 保留至少三个月（事实：永久保留无清理，已满足）。
- 调研事实：sys_operation_log/sys_login_log 两表无逻辑删除无 TTL；41 个 @AuditLog 点；AuditService 全部接口 requireSuperAdmin；无导出无删除；GoodsService.update 等业务写接口放行超管。
- 决策：D76 日志可读性（写入时人话描述+前后快照）；D77 超管真只读（页面放开+后端写权收回，保留七项治理写权，财务维持排除）；D78 日志手动删除（物理删除+留痕，三形态）；D79 xlsx 导出跟随筛选；D80 永久保留无自动清理。挂起：异常操作提醒整项（阈值草案 500/±30% 作废）。

#### 后端
- [ ] B1 调研清点：or-superadmin 放行的业务写接口全清单（真只读收口范围）
- [ ] B2 db.sql 18.x：sys_operation_log 加 detail 列 + 本地执行
- [ ] B3 @AuditLog 加 detail()（SpEL）+ 切面生成描述、填充 before/after 快照
- [ ] B4 41 个注解点补 detail 表达式
- [ ] B5 AuditService：删除（单条/批量 ids/按筛选条件）×两表 + 删除留痕 + 操作日志 xlsx 导出端点
- [x] B6 真只读收紧：按 B1 清单逐个拦截超管（保留：用户/公告/配置/安全策略/价格偏离/作废/部门审批）——17 服务 66 处 `requireNotSuperAdminForBusinessWrite` 首行插入；修复 2 处 replace_all 误伤读路径（PickListService.getById / EmployeeService.getById）
- [x] B7 单测（描述生成/快照/删除留痕/导出/只读拦截）——AuthzServiceTest 4 + AuditServiceTest 9 + AuditLogAspectTest 7 + GoodsServiceTest +1；225/225 全绿（191→+34）

#### 前端
- [x] F1 OperationLogView：操作描述列（历史记录兜底 模块·动作）+ 动作/目标类型筛选 + 详情抽屉重排（原始字段 + 请求/返回快照 pre 块）+ 删除三形态 + 导出按钮（JSON 错误体识别）
- [x] F2 LoginLogView：删除三形态（单条/批量/按条件，空筛选前端拦截）
- [x] F3 router 白名单放开全部业务模块（财务/HR 图表除外，动态详情前缀匹配）+ layout 三个只读分组菜单；`v-permission` 指令改超管语义——须显式含 'superadmin' 才放行（原 admin 蕴含规则使超管可点全部业务按钮 + `{deptCodes}` 裸绑定盲区，一并堵死）；路由级 hasRole 语义不动
- [x] F4 build + 重启 + E2E：超管写 403（建商品/销售确认/生产打卡）+ 读全绿（15 模块 page/getById，含 2 处误伤修复回归）+ detail 落库与密码脱敏（重置密码实测 `{"newPassword":"******"}`）+ 删除三形态与留痕 + 空筛选 400 + 非超管删日志 403 + xlsx 导出（PK 头/UTF-8 文件名）

### 增量优化：侧边栏菜单滚动 + 手风琴展开（2026-09-13 会话 32，grilling 一轮定案）

- 背景：阶段 22 浏览器手测（3 项全过，闭环）中发现——超管全展开菜单后超出视口看不到底部项，还需手动逐个收起分组。根因：`.app-aside` overflow:hidden 裁切 + 无滚动出口。
- 定案：① 品牌块固定 + 菜单区独立滚动（flex 列布局，6px 半透明细滚动条，不引 el-scrollbar）；② el-menu 加 `unique-opened` 手风琴（展开一组自动收起其他组，defaultOpeneds 路由联动不受影响）；③ 全角色统一生效。
- 落地：layout/index.vue 3 处（unique-opened / .app-aside flex 列 / .app-menu flex:1+overflow-y:auto+滚动条美化）；`npm run build` ✅ + 用户手测 5 点全过（滚动到底/手风琴/路由联动/折叠浮层/其他角色回归）。纯 UI 修复，无 ADR。

### 阶段 23：库存盘点模块（D81–D85 / ADR-0010，2026-09-13 会话 32 grilling 三轮定案；2026-09-14 会话 33 手测闭环）✅ 完成

- 背景：客户需求文档痛点 4「盘点耗时费力，循环盘点难以执行」+ 痛点 1「账实不符」的直接纠正手段；阶段 4 三子项（盘点/余料/追溯）中痛点最直给、范式最成熟者先行。客户 2000㎡ 平面库、人工计件。
- 决策（D81–D84，详见 ADR-0010 与 CONTEXT.md「盘点」节 8 词条）：**D81** 盘点单自选范围（物料+成品，可全选=全盘），主从表四态（盘点中1/待审核2/已完成3/已取消4），明细快照名称/规格/材质/单位，部分提交（≥1行）未盘行不调整，「上次盘点时间」查询派生辅助人工循环盘（自动排程留二期）；**D82** 业务不冻结，**差异=实盘−生效时点实时账面**（非快照，防并发重复调整），明细存快照/实盘/生效时账面三值；**D83** 已完成终态不可作废、无删除仅取消、无审批层（录入≠审核双人制衡+审计留痕）、无站内消息、明细无金额列；**D84** 盲盘导出 xlsx（默认无账面数可切明盘）+ 导出表即回填模板导回批量回填 + 页面逐行录入双通道，权限=仓储 admin 建单/审核/驳回/取消 + 仓储员工可录入/**提交**（2026-09-13 手测后修订：初版提交也收口 admin，员工实盘完应可直接送审，审核仍收口 admin）+ 超管只读。

#### 后端
- [x] B1 db.sql 19.x：biz_stocktake + biz_stocktake_detail 建表（明细三值+快照四列+goods_code）+ 本地执行
- [x] B2 Entity BizStocktake + BizStocktakeDetail；Mapper ×2（含 selectLastStocktakeTimes 派生查询）
- [x] B3 DTO（Create 商品勾选/Entry 实盘录入/Query/Reject/Cancel）+ VO（StocktakeVO/StocktakeDetailVO/StocktakeGoodsOptionVO 带上次盘点时间+开单占用标记）
- [x] B4 CodeGenerator.stocktakeNo() = ST+时间戳
- [x] B5 StocktakeService：page/getById/create(快照+开单排他)/entry(逐行+xlsx导入回填)/submit/review(生效时点实时账面差异调整，私有 increaseStock/decreaseStock)/reject/cancel/export(盲盘默认)/goodsOptions
- [x] B6 Controller /business/stocktake/*（@RequireAdmin 不含 entry/import、@AuditLog detail SpEL、@PreventDuplicateSubmit、无 DELETE 端点）
- [x] B7 单测（TDD 先行 22 例 RED→GREEN）：快照/开单排他/部分提交/实时账面差异三值/未盘行不动/驳回重录/取消零库存/终态守卫/权限/上次盘点时间派生/盲盘导出列/导入错误行——全套 247/247 绿

#### 前端
- [x] F1 API 封装 stocktake.js（blob 导出 + multipart 导入）
- [x] F2 StocktakeView.vue：列表+筛选+建单对话框（商品多选、类型筛选、最久未盘排最前、未完结单商品禁选）+ 详情/录入（行内实盘、差异标色行、盲盘导出/导入回填/保存录入/提交审核/取消）+ 待审核预估汇总提示「以审核生效时实时库存为准」+ 已完成三值与盈/亏/平/未盘汇总
- [x] F3 路由 + SUPERADMIN_ALLOWED 白名单 + 仓储 admin 菜单「库存盘点」+ 超管只读分组 + 仓储员工菜单块（D84）
- [x] F4 build ✓ + 重启 ✓ + curl E2E 全通过（建单→录入→盲盘导出无账面列→导入回填/未知编码拦截→负数拦截→提交→驳回→重提→审核生效库存 3→5 三值正确/未盘行不动→终态守卫全拒→开单排他拦截→取消释放→sales_admin/superadmin/员工权限负测全 403 body→lastStocktakeTime 派生正确）+ 测试数据清理库存复原 ✓ + 留档 ✓

### 阶段 24：补料再补解锁 + 部分到货仍缺料通知（D86–D87 / ADR-0011，2026-09-14 会话 33 grilling 两轮定案）✅ 完成

- 背景：阶段 16 登记已知问题的立项修复——补料单按行部分到货→仓储确认入库（整单终态）→生产单仍缺料时，受 D59 幂等守卫旧口径（status≠已驳回 即拦截，把终态已入库(3) 也算在途）约束，该生产单永远无法再补（死锁）；且 D62「仍缺料沉默」让生产只能靠刷列表发现。现网实测无处于死锁状态的活跃生产单，纯行为修复无数据迁移。
- 决策（详见 ADR-0011）：**D86** 幂等守卫收窄到在途状态（待采购1/采购中2/待入库确认5，常量 `IN_FLIGHT_STATUSES`），已入库(3)/已驳回(4) 不占名额，防重复真正闸门是既有「当前无缺料无需补料」实时缺口重算；同一生产单在途补料单仍至多一张。**D87-a** 详情 VO 透出 `inFlightRequestNo`，补料弹窗顶部提示「已有在途补料单」并禁用提交（消除"填一屏才被拦"）。**D87-b** confirmReceive 重算仍缺 → 发「补料部分到货仍缺料」通知（含缺口摘要+可继续补料指引，绑 production_order + target_route），与「物料已齐套可领料」互斥——重算时先按标题白名单 `revokeUnreadByBizAndTitles` 撤旧未读再按结果发新，同 biz_type 的「关联销售单已取消」(D73) 不受影响、不堆积（修订 D62 沉默口径，D59/D62 本体不动）。

#### 后端
- [x] B1 PurchaseRequestService：`IN_FLIGHT_STATUSES` 常量 + `listNonFinalByProductionOrder`→`listInFlightByProductionOrder`（D86）+ `notifyKitCompleteIfReady`→`notifyKitAfterReceive`（重算缺口，撤旧发新二选一，D87-b）
- [x] B2 MessageService：`TITLE_KIT_COMPLETE/TITLE_KIT_INCOMPLETE/KIT_FAMILY_TITLES` 常量 + `revokeUnreadByBizAndTitles`（标题白名单撤未读）+ `sendKitIncompleteToProductionAdmins`
- [x] B3 ProductionOrderService.getById 透出 `inFlightRequestNo`（D87-a，VO 加字段）
- [x] B4 单测（TDD 先行 5 例 RED→GREEN）：守卫 SQL 段只含 1/2/5 不含 3 / 齐套时 InOrder 先撤后发 / 仍缺发仍缺料通知（摘要含物料×缺口）且不发齐套 / 白名单撤销不碰「关联销售单已取消」/ getById 透出在途单号——全套 260/260 绿（255→+5）

#### 前端
- [x] F1 ProductionOrderView 补料弹窗：打开时读 `inFlightRequestNo`，有在途单则顶部 warning 提示 + 提交按钮禁用（缺口行仍可查看）
- [x] F2 build ✓ + curl E2E 原死锁场景全链路（部分到货→确认入库→仍缺料通知内容正确→**再补 200 成功（D86 解锁点，旧口径此处 400）**→在途时再补 400 拦截→全部到货→撤旧仍缺料通知+发齐套通知→stock 47→50 数学正确）+ sales_admin 权限负测 403 body ✓ + 测试数据清理库存复原 47/47 ✓（作废单 34 触发 D21 撤齐套通知、删 BOM/成品走 API 守卫）+ 留档 ✓
...
| 编号 | 决策内容 | 理由 | 日期 |
|------|----------|------|------|
| D1 | 客户需求以 `wms_v1.docx` 为唯一来源（**2026-09-14 修订**：文档部分需求已过时，需求以用户讨论拍板为唯一活来源，`document/` 仅历史参考、不再作为需求依据） | 用户明确指示，不考虑上一轮其他文档；修订因多轮讨论后文档与现场决策脱节 | 2026-06-28 |
...
| D76 | 操作日志可读性：@AuditLog 写入时生成「操作描述」人话摘要落库新列 + before_data/after_data 开始赋值（关键字段变更前后快照）；前端列表加描述列、补动作/目标类型筛选，原始字段收详情抽屉，历史记录兜底显示模块+动作；不做 requestUri 前端映射翻译。ADR-0009 | 描述写入时定格不怕路由/参数改名；前端翻译无数据快照且路由一改即失效 | 2026-09-13 |
| D77 | 超管=治理/审计角色，业务数据真只读：前端白名单+菜单放开全部业务模块页面（按钮隐藏），后端业务写接口统一拒绝超管（含收回 GoodsService.update 编辑权）；保留写权仅用户管理/公告/系统参数/安全策略/价格偏离审批/作废审批/部门审批；财务两模块（利润分析/年度经营统计）维持显式排除。ADR-0009 | 仅前端只读挡不住直接调 API；公司要"超管看所有人、自己不沾业务"；成本利润仅财务可见的有意设计不动 | 2026-09-13 |
| D78 | 操作日志+登录日志加手动删除：单条/批量勾选/按筛选条件清空（二次确认+显示条数），物理删除不可恢复，删除动作本身写一条操作日志留痕；仅超管。ADR-0009 | 无自动清理（D80）不等于放弃手动治理权；审计表逻辑删除无意义，删了就该真删；删日志是敏感操作必须留痕 | 2026-09-13 |
| D79 | 操作日志 xlsx 导出跟随页面筛选条件（对齐 ADR-0008 技术栈），仅超管；登录日志不导出。ADR-0009 | 所见即所导最直觉；登录日志无导出需求 | 2026-09-13 |
| D80 | 日志保留策略：永久保留、不做任何自动清理机制。ADR-0009 | 审计数据价值在长期追溯，年几万行存储无压力；要瘦身走 D78 手动按条件清理 | 2026-09-13 |
| D81 | 盘点单自选商品范围（物料+成品，可全选=全盘），主从表四态（盘点中/待审核/已完成/已取消），明细快照名称/规格/材质/单位（D60/D63 范式）；允许部分提交（≥1行），未盘行不调整库存；「上次盘点时间」查询派生（最近审核生效行）辅助人工循环盘，自动排程留二期。ADR-0010 | 客户 2000㎡ 人工计件，分区轮盘是循环盘的自然用法；派生口径无双写漂移；自动排程无定时任务基建、一期不值 | 2026-09-13 |
| D82 | 盘点期间业务不冻结；**差异=实盘−生效时点实时账面**（非建单快照，快照仅展示参考），明细存快照/实盘/生效时账面三值。ADR-0010 | 快照基准下盘点期间出入库会被重复调整；冻结 2000㎡ 仓库全天作业不现实 | 2026-09-13 |
| D83 | 盘点单已完成终态不可作废、全模块无删除仅取消；无跨部门审批层（录入≠审核双人制衡+@AuditLog 留痕）；无站内消息；明细无金额列。ADR-0010 | 作废=反向再动库存多错账路径，盘错应再盘纠正；盘点是审计敏感区不该有"消失"路径；仓储内部校正无需审批，框架现成可后补 | 2026-09-13 |
| D84 | 盲盘导出 xlsx（默认无账面数防抄数，可切明盘）+ 导出表即回填模板导回批量回填（BOM 导入范式）+ 页面逐行录入双通道；建单/审核/驳回/取消=仓储 admin、录入/**提交**=仓储 admin+员工（手测后修订：初版提交收口 admin 不合理，员工实盘完应可直接送审）、超管只读。ADR-0010 | 「盘点耗时费力」一半在逐行手敲，导出→填数→导回闭环直击痛点；盲盘是防作弊标准做法；「录入+提交≠审核」双人制衡 | 2026-09-13 |
| D85 | 盘点责任到行+全程操作人留痕：明细行建单逐行指定负责人（每行必选，候选=仓储启用成员）；员工限录/限导本人负责行、admin 兜底可录任意行；盘点中 admin 可改派；行级盖实际录入人+时间章（页面/导入同章），主单盖提交人/取消人章。负责人=计划、实际录入人=实际，分离才完整。ADR-0010 决策 8 | 用户手测后提出「谁填的实盘数、谁提交的审核要能追溯」+「建单时指定谁负责哪些物料」；代盘必须经改派或 admin 代录，两者都留痕 | 2026-09-13 |
| D86 | 补料幂等守卫口径收窄到在途状态：`createDraft` 幂等查询只含 待采购(1)/采购中(2)/待入库确认(5)（`IN_FLIGHT_STATUSES`），已入库(3)/已驳回(4) 不占用名额；同一生产单在途补料单仍至多一张。ADR-0011（修订 D59 实现口径） | 旧口径 status≠已驳回 把终态已入库也算在途，行级部分到货+整单确认后生产单仍缺料却永远无法再补（死锁）；防重复的真正闸门是既有「当前无缺料无需补料」实时缺口重算，宽口径守卫是冗余且有害的 | 2026-09-14 |
| D87 | ①补料弹窗在途提示：详情 VO 透出 `inFlightRequestNo`，弹窗顶部提示「已有在途补料单，待其入库后可再次补料」并禁用提交。②部分到货仍缺料通知：confirmReceive 重算仍缺→发「补料部分到货仍缺料」（缺口摘要+可继续补料指引，绑 production_order+target_route），与「物料已齐套可领料」互斥——按标题白名单 `revokeUnreadByBizAndTitles` 撤旧发新，「关联销售单已取消」(D73) 等同 biz 其他通知不受影响。ADR-0011（修订 D62 沉默口径） | 守卫收窄后"填一屏才被拦"变成可预期提示；仍缺料沉默在"补不了"语境合理（D62 当时语境），能再补后沉默变体验断点；白名单撤销保证互斥不堆积又不误伤其他通知 | 2026-09-14 |
| D88 | 阶段 25 全系统测试环境基线与测试计划（grill 两轮七题全按推荐）：①备份全库至仓库外 `~/wms-db-backups/`（mysqldump 单事务，结构校验 33 表+3 视图+1 存储过程）；②清空仅留账号体系 4 表（sys_user/sys_dept/sys_employee/sys_ip_policy），其余 32 表 TRUNCATE（Q1=A：主数据也清，测试从建供应商/建物料开始）；③账号清理：删 E2E 残留 `e2e_audit_temp` + 物理清除 3 个已逻辑删除的 hxl 系列测试账号；④空库冒烟基线 45/45 + 超管业务写 403；⑤测试计划 `docs/test-plan.md`：P0 冒烟/P1 负测/P2 边界消息三层（Q3=C）、按业务流串联执行（Q4=B）、范围除 AI 外全模块（Q5=B，本地无 LLM 配置）、混合执行（Q2=C：Claude 先接口冒烟，用户按文档手测，问题记回文档）；⑥测后产出《系统使用手册》（素材=计划冒烟层改写） | 主数据本身是待测模块，留着就测不到建档与「名称+规格」唯一性校验；模块间硬依赖（无入库则无库存可出库）决定业务流顺序顺带覆盖集成；历史踩坑集中在读写权限分离与消息悬挂（D21），故负测/消息层单列；逻辑删除账号占 uk_username 唯一键，物理清除顺带解决同名撞键 | 2026-09-14 |
| D89 | 供应商列表联系人搜索命中展示：联系人搜索命中非主联系人时，行「联系人/职务/联系电话」三列切换展示**命中联系人**（同一供应商多命中时主联系人命中优先、否则 id 最小者），姓名旁加「命中」tag；**纯前端实现**（分页行自带 `contacts` 数组，`SupplierView.pickMatchedContact` 挑选覆盖展示），后端零改动、扁平字段语义恒为"主联系人"；前端未匹配到（英文名大小写等）静默回退主联系人；清空搜索即恢复，无落库。CONTEXT.md「命中联系人展示」 | 列表主体是供应商，搜索只决定"替它出镜的联系人"——一行一联系人则行语义翻倍、分页计数变怪；命中 tag 防"把命中联系人误当主联系人"；展示态切换纯前端职责，分页行本已带全量 contacts，不值得为此动 API 语义 | 2026-09-15 |
| D90 | 前端业务错误提示单通道（ADR-0012，根治问题 #1 静默吞错）：request.js 响应拦截器全局识别业务错误（HTTP 200 + body code≠200）统一 toast 并 reject——拦截器是**唯一**提示通道，页面 catch 不再手写错误提示；单请求 `{ silent: true }` 豁免（仅 MessageCenter 15s 角标轮询 + 工作需求附件批量加载按个降级两处）；blob/arraybuffer 透传，导出通道由 `saveBlobAs` 探测 JSON 错误体抛普通 Error，页面 catch 以 `!error?.isAxiosError` 区分业务错误（页面提示）与传输错误（拦截器已提示）；el-upload 双通道保留（http-request 手动回调 + WorkRequirementDetailView 原生 :action XHR 各自页面侧识别 code）；39 文件清扫：172 处死 `res.code` 检查删除、约 130 处重复错误 toast 清除、31 处 `.catch(() => {})` 保留并统一注释（吞 ElMessageBox 取消 + 已统一提示的业务错误）；成功 toast / 本地校验 toast / 状态回滚与 finally 一律保留。CONTEXT.md「业务错误」 | 问题 #1 是两层失败模型：拦截器不识 body code 逼出 172 处手写检查（漏一处即无声失败），吞取消的空 catch 又顺带吞了业务错误；单通道后页面零错误样板、错误必达用户；silent 豁免防轮询/批量场景 toast 轰炸；保留 31 处空 catch 因 ElMessageBox 取消必须吞，注释统一后语义自解释 | 2026-09-15 |
| D91 | 供应商筛选下拉加 `filterable` 打字过滤（Q1=A）：物料管理页（GoodsView）+ 库存预警中心页（StockWarningView）两处搜索区供应商下拉（全站仅此两处），仍按 id 精确查单家供应商，零后端改动；不改自由文本 LIKE（Q1 否掉 B：一对多命中变查询语义且逐页动后端）。进货/退货两页供应商本就是自由文本模糊（单据搜索场景），不动 | 「输入文字模糊匹配」的体验 filterable 完全满足且语义不动；选项一次性全量加载（启用供应商按名称排序不分页，几十~几百条），本地过滤够用，项目已有大量 filterable 先例无 remote 必要 | 2026-09-16 |
| D92 | 首页预警卡片可点直达预警中心（Q2=A 带过滤跳转 + Q4=A 员工放行）：AdminHome/EmployeeHome 的「低库存预警」「零库存预警」指标卡点击 → `/business/stock-warning?type=low|zero`（目标页已有 `applyRouteQuery` 支持该参数，首页计数与预警页过滤口径一致均排除成品）；预警中心读权限放开四部门成员——前端路由 `roles` +employee + 后端 `GoodsService.requireGoodsPageAccess` warningOnly 分支 admin 级→成员级（`requireAnyDeptMemberOrSuperAdmin`），卡片悬浮范式沿用各首页既有 hover；超管首页无预警模块不涉及。顺带修正：StockWarningView `isWarehouseAdmin` 原仅判部门，放开员工后改判 `admin+warehouse`（供应商筛选/工作台标记不误入员工视图） | 点预警卡片的意图就是「看看是哪些货」，带过滤直达才有意义；预警列表数据=成员本就能看的物料列表预警子集，无数据越权；员工首页卡片要能点，路由+后端两道 guard 必须一起放（只放前端会 API 403） | 2026-09-16 |
| D93 | 员工菜单手术（Q5/Q6 均按推荐）：①四个员工菜单块（生产/销售/采购/仓储）移除「用户部门管理」——该路由 `roles: ['admin','superadmin']` 本就不含员工，菜单项是点了 403 的死链，纯清理，admin 同名管理页不动；②仓储员工菜单新增 物料管理/成品管理/预警中心（顺序：物料/成品/库存盘点/预警中心），页面门控天然只读（GoodsView `isWarehouse=admin&&warehouse` 员工拿不到新增/编辑/删除，预警中心供应商筛选仅仓储 admin），Q6=A 不开放写操作；其他三部门员工维持「预警中心可点卡片直达、无菜单」 | 死链菜单是历史遗留（员工从来进不去该页）；库存变更唯一入口=业务单据，仓储员工写职责（盘点录入 D84/D85）已有专门入口，这三页定位是「看得见」 | 2026-09-16 |
| D94 | 作废功能保留 + 解惑包（Q3=A）：用户疑「退货+1库存、作废又退回去」为 bug，经 grilling 确认为**正确设计**（作废=此单不该存在/纠正录错，与业务后悔走新单据是两条正交路径，CONTEXT 词条早有定论），根因是交互零解释（裸 prompt 只问原因、无后果说明、提交后行内无状态）。解惑包三件：①六张单据页统一换 VoidConfirmDialog 说明弹窗（含义/库存影响逐行带商品名数量方向/生效链路/留痕；审批流四类单原因必填，直废两类选填+立即生效措辞）；②审批流四页（进货/购退/销售/销退）行内「作废审批中」状态——新端点 `GET /system/approval-orders/pending-void-biz-ids?bizType=`（@RequireAdmin，透 status∈{1待审批,4处理中} 且 action∈{void,void_red} 的 bizId），状态列加 warning tag + 作废按钮禁用 + tooltip，仅 admin 侧拉取与按钮可见性一致；③删除/作废按钮加分工 tooltip（当天错单删除不留痕/历史错单作废留痕+审批） | 作废语义无错，错在「点了以后发生什么」完全不可见；弹窗把后果写在点按钮之前，行内状态解决「提交后单据看起来没变化」的困惑；直废型（生产入库/生产任务单）无审批流故无行内状态，弹窗措辞区分立即生效 | 2026-09-16 |
| D95 | 删除/作废口径统一 + 当天已生效单死锁解锁（Q1=A/Q2=A）：用户手测发现「当天建、当天已确认」的单据删不让删（各模块 confirmStatus 守卫）、作废审批也不让提（ApprovalService 当天单一律拦截）——死锁，且前端进货/购退页还亮着必失败的删除按钮。定案干净二分：**删除=当天+未生效（confirmStatus=1）错单（无痕）；作废=已生效或历史错单（留痕+仓储审批）**。后端：①ApprovalService.ensureCanSubmitVoidApproval 仅拦「当天+未生效」（BizDocumentMeta 增 confirmStatus 字段），当天已生效单放行——这是其唯一纠错通道；②SalesService.ensureCanDeleteSales 收口——admin 也不可删已出库销售单（原可无痕直删+回补库存，与进货/销退两套规则，是用户困惑根源之一），报错文案对齐「已出库的销售单不可删除，请走作废流程」，delete 内回补库存分支成为兜底防御。前端四审批型视图 canDelete/canVoid 统一为 isPendingTodayDoc 判定（删除仅当天+confirmStatus=1，作废取反），头部帮助/按钮 tooltip 文案同步新口径 | 错误当天发现就该当天能纠正，等隔夜无道理；同类单据必须一套规则；CONTEXT「删除 vs 作废」词条语义（删除=未生效无痕、作废=已生效留痕）由此在四类审批型单据上真正闭环 | 2026-09-16 |
| D96 | 仓储退货确认页 403 误报根治（Q3=A）：仓储 admin 打开「销售退货入库确认」「物料退货出库确认」页弹「仅销售/采购部门可访问」——页面路由/分页 API/确认接口本就放行仓储，报错来自 onMounted 加载建退货单源单下拉 returnableOptions（销售/采购写守卫），本地 catch 静默但拦截器 toast 已弹出（D90 单通道特性）。定案前端门控：源单选项仅建单角色（本部门成员/超管）加载，仓储不拉，零后端改动（选项只服务建单弹窗，仓储用不到） | 与 D94 loadVoidPendingIds 门控同款范式：按角色拉取辅助数据，而非拉下来再吞错 | 2026-09-16 |
| D97 | 已作废/红冲单据状态列直显终态（Q1=A）：用户手测疑「作废后退货状态列还显示原确认状态，歧义」。四审批型视图（进货/购退/销售/销退）状态列加前置分支——bizStatus=2 显灰色「已作废」、=3 显「已红冲」（红冲行即作废执行时生成的负数对冲单），否则维持原 confirmStatus 标签；纯前端，VO 本就透 bizStatus | 状态列的职责是「这单现在算什么」，终态单据继续显示出库/入库态必然误导读流程的人；action 列 resolveState 已有「作废成功」文案，状态列与之一致 | 2026-09-16 |
| D98 | 作废审批中冻结主流程（Q2=A 全冻结）：用户手测踩出数据腐化通道——购退单仓储出库确认后提交作废，审批还挂着，采购侧「确认退货成功」按钮仍可点（点下会继续推进 workflow+动库存）。探查发现 6 个确认方法中 4 个连 ensureNormalStatus 都没有（PurchaseService.arrive/confirmReceive、PurchaseReturnService.confirmOut/complete）。定案双层：①前端四视图确认按钮 bizStatus≠1 直接隐藏、作废审批中（voidPendingIds）禁用+tooltip「作废审批中，待仓储管理员处理」，且 loadVoidPendingIds 门控从「仅本部门 admin」拓宽为「本部门 admin + 仓储 admin」——确认按钮主操作者是仓储 admin（端点 @RequireAdmin 任意部门 admin 可过、无 dept 细分，员工仍 403 不变量保持）；②后端四 Service 各加私有 ensureNoPendingVoidApproval（查 biz_approval_order 该单 status∈{1,4} 且 action∈{void,void_red} 即拦），6 个确认方法全部接入+补齐 ensureNormalStatus | 前端禁用是体验，后端守卫是数据完整性——审批未决期间单据归属未定，继续推进 workflow 等于让作废审批失去意义；守卫按单查询轻量，确认动作本就不高频 | 2026-09-16 |
| D99 | 红冲全面停用+措辞通俗化（Q2=A+用户直答）：用户看到 D97「已红冲」质疑「之前讨论过不要红冲」——查证留档实为 D74（2026-09-10）**隐藏入口、后端保留**，且 D97 选项文案带「已红冲」未向用户标注这层历史（沟通失误，红冲字样第二次冒头）。全量实证：前端 4 视图硬编码 requestAction='void' 零红冲出口、全库 biz_status=3 存量=0。定案：①措辞——界面统一「红冲」→「冲抵」（状态标签「已冲抵」+悬浮说明、resolveState「作废并冲抵成功」、审批中心历史 action 文案、登录页/首页提醒/年报 tooltip、后端报错「为冲抵记录」「请走作废流程」、审计日志 action 收敛为「作废」）；②机制——审批提交 validateAction 拒绝 void_red + 5 个 Service voidDocument 拒绝 createRedFlush=true（直废接口的隐藏第二路径，一并焊死），报错「『作废并冲抵』已停用，请提交普通作废」；生成逻辑代码保留为死代码（D74 口径），普通作废的库存复原不受影响 | 「入口隐藏+机制保留」的折中被证明守不住——显示层总要描述这些状态，与其每次向用户解释，不如把产生路径全部焊死，让「不要红冲」永久成立；「红冲」是财务黑话，「冲抵」是日常词（用积分冲抵押金），历史行加悬浮说明杜绝第三次困惑 | 2026-09-19 |
| D100 | 缺省供应商锚点复活+禁删守卫（Q1=A 根因修复）：用户报「仓储端新增成品提示供应商不存在」。根因：GoodsService.DEFAULT_SUPPLIER_ID=1 魔法常量把成品建档/未知物料自动建档挂靠到 base_supplier id=1，而该行在会话 31 E2E 供应商清理中被逻辑删除——selectById 查不到即报「供应商不存在」，属测试残留数据事故而非代码回归。修复三件：①DB 修复 id=1 行（is_deleted=0、更名「系统默认供应商」）；②SupplierService.delete 加守卫——删除 DEFAULT_SUPPLIER_ID 抛「系统默认供应商是成品建档与自动建档的系统依赖，不可删除」（守卫置于 requireSupplier 之前，行被逻辑删除也能拦）；③db.sql 种子数据新增 SUP000「系统默认供应商」行置于 SUP001 之前，保证全库重建后 id=1 仍是专用锚点（否则 id=1 会落到华强电子） | 魔法常量依赖一条可被删除的主数据行，删除守卫+种子行把这条隐式依赖变成显式契约；锚点行改名「系统默认供应商」让列表里看得见它是什么 | 2026-09-19 |
| D101 | 直接进货确认入库回写最新进价 + 作废重算（Q2=A/Q5=A）：用户报「同一物料多次采购价格不同，界面应展示最后一次」。排查发现两条采购链路只有采购申请链路（createInternal）回写 base_goods.purchase_price，直接进货单 confirmReceive 漏写（实证：物料顶盖01 九一六进货 ¥5 后进价仍 NULL）。修复：①confirmReceive 增加 LambdaUpdateWrapper 单列回写 purchase_price（不用 updateById 整行覆盖——increaseStock 已 SQL 自增 stock，整行覆盖会竞态回写旧库存）；②作废已入库进货单后 refreshPurchasePriceAfterVoid 重算为 latestValidUnitPrice（最近一笔有效已入库批次单价），无更早有效批则保持原值不动 | 「最近一批进价」口径收口为两条链路统一写、作废链路重算；单列更新规避 updateById 与 increaseStock 的整行/自增竞态 | 2026-09-19 |
| D102 | 物料进价历史弹窗（Q2=A 保留前几次记录）：物料管理「进价」列（仅采购部门成员/超管可见，沿 showPrice 门控）加「历史」链接按钮，弹窗展示该物料全部有效已入库采购记录（进货单号/单价/数量/总金额/进货时间/入库时间，最近在上 LIMIT 100）；后端新端点 `GET /base/goods/{id}/purchase-price-history`，GoodsService.purchasePriceHistory 服务端把关 requireDeptMemberOrSuperAdmin(DEPT_PURCHASE)——仓储 admin 403，进价可见口径不因新端点破口 | 当前价列展示「最后一次」、历史弹窗保留「前几次+时间」，正合用户诉求；权限在服务端把关而非前端藏按钮，防止直调 API 泄价 | 2026-09-19 |
| D103 | 作废审批站内信化 + 移除右下角浮卡（Q3=A/Q4=A）：用户报「作废审批提示不要右下角浮窗，改成和其他提醒一样」。定案：①提交作废申请 → MessageService.sendVoidApprovalPendingToWarehouseAdmins 发「待审批作废申请」站内信给仓储管理员（D21 范式绑 biz_type/biz_id，targetRoute 指向作废审批页）；②approve/reject 终态 → revokeUnreadByBizAndTitles 撤该待办（reject 用标题白名单撤、避免误杀同 biz 的待确认销售等无关待办；approve 由 executeVoidByApproval 的 revokeUnreadByBiz 顺带撤后再发回执防误杀）+ sendVoidApprovalResultToRequester 回执申请人「作废审批已通过/已驳回」（驳回带原因）；③AdminHome 移除「作废审批提醒」右下角浮卡（approvalReminder 加载/签名去重/展示块全删），后端 pending-reminder 端点保留为无害读接口 | 与其他业务提醒一致走站内信+角标，右下角浮卡是首页自建的第二套提醒样式（孤例）；回执在撤销之后发送的时序保证回执不被当作待办撤回 | 2026-09-19 |
| D104 | 四类跨部门单据流程时间线（grill 一轮四题全按推荐）：用户诉求「物料进货/退货、采购申请、商品退货等跨部门单据都像商品销售那样能看到谁在哪一步做了什么，单据完成后点查看除物料信息还能看到操作流程」。定案：①范围=四类跨部门单据（进货单/采购申请单/进货退货单/销售退货单），生产任务单（详情已有工序列表）与盘点单（部门内）不纳入；②数据口径=**标准流程合成**（与销售履约时间线同模式：按单据字段合成节点，节点=业务步骤+操作人+时间+库存影响；操作人有人名字段用人名，角色锁定步骤用部门名；回退后仅显示当前步骤）——否掉审计日志驱动（需补注解+开权限+历史不全，弯路细节本就属超管操作日志的审计职责）；③作废呈现=尾部「已作废」红色节点（人/时间/原因）+ 作废审批中显示 pending 节点（查 biz_approval_order status∈{1,4}，与 D98 冻结口径呼应）；④组件=抽通用 DocumentTimeline.vue（props: nodes 纯渲染，voided 红色/voided 三态映射），SalesTimeline 渲染段迁移复用。后端：DocumentTimelineService 四方法（读守卫与各 getById 同口径）+ 四 Controller `GET /{id}/timeline` 端点 + DocumentTimelineVO/NodeVO。前端：api 4 条 + 四视图查看弹窗内嵌时间线 | 「像商品销售那样的生命周期」锚定的是销售既有实现模式（字段合成+el-timeline 详情内嵌），用户确认的入口（操作列查看）与内容（谁在哪一步做了什么）直接映射到该模式；时间线（业务步骤视图）与操作日志（审计流水，仅超管）在词汇表分立，语义不混 | 2026-09-19 |
| D105 | 齐套领料显示（grill Q1=用户直答，按单成立）：用户报「生产单已 BOM 录入并齐套领料可开工后，仍显示缺料，应显示齐套领料」。根因：详情/列表按**当前库存**实时重算齐套率，领料出库已把物料库存扣掉，已全额领料的单反而显示缺料。定案口径（用户原话锚定）：齐套按**单**成立——「针对某个领料单，该领料单已全额出库，这个成品就是齐套的；后续再生产是另一张领料单，与此无关」。实现：生产任务单关联领料单全部明细全额出库（pick 状态 ISSUED/DONE 且 allMatch 出库）即 kitStatus=`issued`「齐套领料」、kitLines 清空；列表批量 in 查询 pickIssuedMap 一次装配（无 N+1），详情同口径；显示区间=领料出库 → 生产入库完成（含待入库状态 3）；生产入库完成（状态 4）/作废/报废/终态去标签；未领料或未全额出库维持四态实时口径（齐套/部分缺料/严重缺料/未知物料，状态 3 不重算）；补料重算通知逻辑不动 | 「这张单的料领齐没有」是单据事实问题，不是仓库现在还有多少料的库存问题；用领料出库事实替代库存快照，列表与详情同一数据源根除两处口径漂移 | 2026-09-19 |
| D106 | 销售日期=开单时间自动生成（删日期选择器）：用户报「销售单下单时间与预计出库时间混淆，删掉出库日期，根据开单时间自动生成销售日期」。实证：新建弹窗的「出库日期」选择器绑定字段实为 operationTime（开单/业务时间），标签本身错误；系统无销售单编辑功能。定案：删除新建销售单的日期选择器与 SalesSaveDTO.operationTime，SalesService.create 强制 `LocalDateTime.now()`，前端查看态只读展示销售日期、新建态提示「开单时自动生成（当前时间），不可选择」；「出库日期」标签废除；列表列与搜索项「销售日期」保留（仍按 operationTime 过滤） | 标签把开单时间误叫出库时间是混淆根源；不可改的时间不该有选择器；搜索/列表叫法与需求文档对齐为「销售日期」 | 2026-09-19 |
| D107 | 成品入库两段式确认（grill Q2，对齐销售退货 confirm_status 范式）：用户报「成品入库后自动确认入库，仓储模块缺少确认成品入库按钮」。定案：①生产端「生产入库」按钮改「提交入库申请」——生成 biz_production（confirm_status=1 待确认、biz_status=1），**不加库存**、生产任务单停留状态 3（待入库），详情标注「已提交，待仓储确认」，站内信通知仓储管理员（biz=production，D21 绑全生命周期）；②仓储端生产入库页加「确认入库」（仓储管理员）——乐观守卫（eq id+confirm_status=1，rows≠1 报「已被处理」）确认时才 increaseStock + 任务单转已完成（4）+ 撤待办 + 关联销售单通知销售可发货；③生产可「撤销申请」（逻辑删申请+撤未读），仓储可「驳回」（原因必填，confirm_status=3+回执提交人，回执不撤销）；④仓储手动「新增生产入库」保留直接生效（录入即 confirm_status=2、确认人=录入人）；⑤整单确认不拆量；仓储列表加确认状态筛选与列（待确认/已确认/驳回+原因 tooltip），删除/作废守卫：待确认引导生产撤销、驳回单无库存动作直接放行；存量 confirm_status NOT NULL DEFAULT 2 回填历史为已确认入库；任务单收尾暴露 ProductionOrderService.finalizeAfterInboundConfirmed 同事务调用，规避循环依赖 | 入库即加库存让仓储对成品库存零把关；两段式把「生产报完工」与「仓储验收」拆成两个角色动作，与销售退货/进货验收的既有确认范式对齐；手动录入是仓储自己的录入行为，无需自己确认自己 | 2026-09-19 |
| D108 | **价格偏离阈值设置常驻（2026-09-20 用户纠正后正名）**：用户原话「这是一个功能不是一个数据，阈值多少是超管设置的」指的是**「系统参数」页的价格偏离阈值设置项**——清库时 sys_config 阈值行被物理删除，系统参数页显示「暂无参数」、修改按钮随行消失、更新接口报「参数不存在」，功能看似丢失。定案：①SysConfigService 启动自检自愈——阈值行缺失补默认行 0.05、逻辑删行（唯一键冲突）复活、自愈异常只记日志不阻断启动（内存兜底 5%）；db.sql 种子保留供全新建库；②阈值=超管可配系统参数的领域定位写入 CONTEXT，任意登录用户可读、仅超管可改（原有权限模型不变）。**误读纠正**：会话 42 曾把该诉求理解为「价格偏离审批入口常驻」并做了总览 nav 卡/待审指标/首页蓝色提醒卡/新端点——审批菜单本就常驻，这些非诉求改动已于 2026-09-20 整体回退（5 文件恢复至批次一之前） | 功能设置的存在性不依赖业务数据；系统依赖的参数行系统自持（与 D100 默认供应商种子行同范式）；审批入口本就常驻，不需要为"没有待办"额外造入口 | 2026-09-20 |
...
- 当前阶段：⏸ 阶段 25 全系统测试暂停转优化（会话 35：首轮手测发现多项优化点——先集中优化（新会话收修改清单，Q1=修完一次性回归）→ 环境已重置为账号-only（Q2=空库全量重测）；已确诊问题#1：供应商被引用删除前端静默吞错，`.catch(()=>{})` 同类模式待全站排查）；挂起：异常操作提醒（阶段 22 挂起，阈值草案作废，恢复重议）、移动端适配（2026-09-11 挂起，恢复续 Round 2）、余料管理（2026-09-14 挂起，Round 2 四决策已定，恢复续 Round 3）；搁置：成品追溯（2026-09-14 拍板，成品无非卖出/退回无其他流向）
- 后续候选：① 余料管理（挂起，恢复续 Round 3——范围/登记形态/台账派生/处置集合已定，存 progress 会话 33 末条与记忆）② 异常操作提醒（挂起重议）③ 移动端适配（挂起，续 Round 2）④ 手动退料 createReturn 服务端净额兜底小修（余料调研顺带发现的独立漏洞：ensureWithinReturnable 仅终止路径调用，直调 API 可退未领物料/超量退，可单独立项）
- 阻塞项：无
