...
### 阶段 20（挂起，待公司与业务确认）
- **E1 库存竞争**：建单不预留库存，两张"现货充足"单先出库先赢，后者出库被硬校验拦截、仓储退回销售协调。暂定行为：不预留、不自动通知生产。待确认：是否需要库存预留机制或竞争发生时自动通知生产。
- **E2 关联单作废**：已关联生产任务单的销售单被删除/作废，不拦截，生产单上 sales_order_id 保留（可做完当备货）。暂定行为：不拦截、关联保留。待确认：是否需要"作废前须生产确认"闸口。
...
- 完成度：阶段 1–3.7 + 5–8 + 9–19（D60–D67 含修复轮）+ **阶段 20（销售端三件套 D68–D71）** 编码 100%（后端 + 前端 + E2E 验收通过，154 单测全绿）
- 当前阶段：阶段 20 已交付（2026-09-10 会话 25 实施完成）；候选下一项：① 阶段 4（盘点/余料/成品追溯）② 阶段 16 登记的已知问题（补料单按行部分到货→确认入库终态后，受 D59「已入库补料单阻止再补料」约束，该生产单无法再补——待单独立项）③ E1/E2 确认后的库存预留/作废闸口增量
- 阻塞项：E1/E2 待公司确认（不阻塞阶段 20 实施，按暂定行为落地）
