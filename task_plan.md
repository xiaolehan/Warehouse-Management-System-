...
| D108 | 价格偏离审批常驻入口（用户原话：「这是一个功能不是一个数据」）：用户报「上次清空数据库后超管界面看不到价格审批了」——根因是旧入口只在**存在待审数据**时出现（首页提醒卡/角标驱动），空库时功能不可发现。定案对齐部门审批待遇：①超管总览页 nav 卡片「价格偏离审批」+ 待审指标卡（新端点 `GET /system/approval-orders/pending-price-deviation-count`，ApprovalService.pendingPriceDeviationCount 统计 status=待审 + request_action=price_deviation_confirm）；②超管首页蓝色提醒卡（有数据时出现、可关闭、数量变化重新弹出，复用部门审批卡的 sessionStorage 签名去重模式），点击直达 /system/void-approval；空数据时入口仍常驻、指标显示 0 | 功能入口的存在性不能依赖数据存在；「看不到功能」和「没有待办」是两件事，常驻 nav 卡解决发现性、提醒卡解决即时性 | 2026-09-19 |
