...
public class PurchaseReturnController {
    ...
    public Result<DocumentTimelineVO> timeline(@PathVariable Long id) {
        return Result.success(documentTimelineService.getPurchaseReturnTimeline(id));
    }
    ...
    @AuditLog(module = "进货退货管理", action = "批量删除", targetType = "进货退货单")
    @PreventDuplicateSubmit(intervalMs = 1200, message = "删除请求过于频繁，请稍后再试")
    public Result<BatchDeleteResultVO> batchDelete(@RequestBody List<Long> ids) {
        return Result.success(purchaseReturnService.batchDelete(ids));
    }

    @PutMapping("/{id}/void")
    @AuditLog(module = "进货退货管理", action = "作废", targetType = "进货退货单",
            detail = "'作废 进货退货单 #' + #id + (#dto?.reason != null ? '，原因：' + #dto.reason : '')")
