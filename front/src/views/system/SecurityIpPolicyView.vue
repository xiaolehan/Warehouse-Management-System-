<template>
  <div class="policy-page">
    ...
    <el-card class="panel" shadow="never">
      ...
      <el-table :data="tableData" v-loading="loading" stripe border empty-text="暂无策略数据" @selection-change="handleBatchSelectionChange">
...
import { batchDeleteIpPoliciesAPI } from '@/api/security.js'
...
const handleBatchDelete = async () => {
  try {
    await ElMessageBox.confirm(`确认批量删除选中的 ${batchSelectedRows.value.length} 条策略吗？不满足条件的将跳过并提示。`, '警告', { type: 'warning' })
    const res = await batchDeleteIpPoliciesAPI(batchSelectedRows.value.map((r) => r.id))
    const data = res.data || {}
