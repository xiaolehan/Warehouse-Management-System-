<template>
  <el-card>
    ...
    <el-table v-loading="loading" :data="tableData" border style="width: 100%" @selection-change="handleBatchSelectionChange">
...
import { batchDeleteWorkRequirementsAPI } from '@/api/workRequirement.js'
...
const handleBatchDelete = async () => {
  try {
    await ElMessageBox.confirm(`确认批量删除选中的 ${batchSelectedRows.value.length} 条工作要求吗？不满足条件的将跳过并提示。`, '警告', { type: 'warning' })
    const res = await batchDeleteWorkRequirementsAPI(batchSelectedRows.value.map((r) => r.id))
    const data = res.data || {}
