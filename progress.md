...
- **票面**：`.scratch/manual-test-2026-09-23/issues/`（01 改名 / 02 主数据批删 / 03 单据批删 / 04 系统页批删，全部 ready-for-human）。
- **grilling 定案**：批量删除范围=所有删除页（15 页）；部分失败=尽力而为（逐行校验，能删的删，失败报明细）；物料管理「产品名称」=归属成品列确认；成品删除权=主数据编辑权（售价编辑不算）。
- **改名票（纯前端 5 处）**：生产入库表头+弹窗「商品名称」→「成品名称」且检索由入库单号改为按成品名称（`productionNo`→`goodsName`，后端 like 已有）；物料管理检索/表头/弹窗「产品名称」→「成品名称」；销售出库「出库商品」→「出库成品」；销售退货「退回商品」→「退回成品」；预警中心「商品名称」→「物料名称」。
- **批量删除后端（15 模块）**：新增 `vo/BatchDeleteResultVO{successCount,failureCount,failures[{id,name,reason}]}`；每模块 `POST /xxx/batch-delete`（body `List<Long>`）——请求级守卫一次先行，逐行执行单删同款校验（两种实现：Goods/Sales/SalesReturn/Purchase/Bom 提取 `deleteInternal`；其余 batchDelete 循环调用原 delete，守卫幂等）。BOM 批量走非强制（有未完结任务单的行进失败明细，强删走原单条流程）；D21 消息撤销逐行生效。
- **批量删除前端（15 页）**：复刻登录日志范式——多选列 + 「批量删除（N）」按钮 + 确认弹窗 + 结果提示（成功 N 条 / 失败 N 条带明细）。权限镜像各页单删（BOM=生产、供应商=采购、销售=admin+employee、采购申请/领料按钮文案「批量撤销」）；系统页（用户/员工/公告/工作要求/IP）无 v-permission，页面级 admin/超管 + 行级失败明细兜底。API 批量函数 15 个分布 7 个 api 文件。
- **验证**：`./mvnw clean compile` BUILD SUCCESS；单测新增 9 例（守卫整单拒绝/混合成败聚合/缺失行报告）套件 **422/422 全绿**；`npm run build` 通过；E2E——仓储建 2 物料批删 `[110,99999,111]`→2 成功+1 失败（99999：商品不存在）、仓储批删 BOM→403 仅生产可维护、生产建采购申请后批撤销 `[1,99999]`→1 成功+1 失败；测试数据已清理。
- **部署**：重打包 jar 并重启 8080（pid 更新）；4173 preview 直接服务新 dist（改名文案已进构建产物）。
- **备注**：Notice/Security 无既有单测文件，本轮未补（控制器守卫+行级校验由 E2E/人工覆盖）；UserManage 批删复用 `requireManageableUser` 行级授权，超管保护行进失败明细。
...
- **小票（先做）：** D115 认领采购申请到货备注「应用到全部」（对齐时间填充）；D118 仓储盘点列表导出盘点表；D117 生产确认收货入口并入生产任务单详情（开工前可见，按申请人 id 判定，user store 暴露 userId）；D116 齐套状态实时重算（全部入库即「齐料」）；D119 进货退货查看采购端/仓储端数据不一致 → 统一为同快照（D119 并入票 01）。
- **票 01/D111（ADR-0015）：** 进货单头行结构——`biz_purchase_detail` 一单 N 物料行、uk_purchase_goods(purchase_id,goods_id)；头表只留单据+汇总；多行建单不碰库存，到货/确认入库**逐行**加库存+**逐行回写进价**；`biz_purchase_return` 对称多行化，uk_return_source_line，按来源进货明细行退，行级可退量=行入库量−已实际出库退货累计，confirm-out 逐行扣库存；**整单粒度**作废/删除（v1 不做行级作废），删除级联软删明细行；作废已入库单逐行回冲+逐商品进价重算（latestValidUnitPrice 下沉明细行）；进价历史改读明细行；统计/引用统计全部 detail JOIN head。db.sql 规范段 3.1/3.1.1/3.2/3.2.1、视图 5.2、种子 4.6/4.7、六处成本快照子查询全部同步；增量段 22 含完整可执行迁移+视图重建。
- **单测：** PurchaseServiceTest 新建 8 例、PurchaseReturnServiceTest 新建 11 例；**全量 319 全绿**（基线 300+19；含 D117 等适配）；前端 build clean。
- **E2E（/tmp/d111_e2e.py）19/19：** 多行建单/重复物料拦截/头汇总/逐行入库+库存/行级退货/重复来源行/超退上限/跨单拦截/汇总列/作废守卫拦截→删退货→作废放行/作废后库存还原。测后清理：单据软删、库存完全还原。
- **/code-review 修复：** ①票 01 增量段视图重建仅注释指引 → 补可执行 CREATE OR REPLACE VIEW（本地实证 40 行）；②已入库进货单在有未终结退货单时作废会按整行回冲、与已出库退货冲突 → PurchaseReturnService.ensureNoActiveReturn 守卫，拦截并列出退货单号给可操作提示（先删/废退货再废进货），PurchaseService.voidDocument 写库前调用；③退货路径误加商品启用校验（scope creep）→ 移除，停用/停产物料也可退；④D117 申请人按显示名判定 → 按 id。
- **有意识留后（judgement calls）：** avgPrice 命名（与 SalesVO 对称，改名破坏一致性）；create/createInternal 重复与 buildGoodsSummary 重复（票 02/D120 将大改进货到货区，避免同区两轮重构）；进价历史 LIMIT 100 在 Java 侧（SQL 下推需 JOIN 自定义 SQL，单物料行数有限）。
- **下一步：** 票 02/D120（按行分批到货，ADR-0016，被 D111 阻塞已解除）→ 03/D113 → 04/D112 → 05/D114；全部完成后全量测试+build+终审+提交，交用户统一手测（补测项已记 pending-retest.md「D111+ 联动复测项」）。
...
- **拍板：** 首轮手测（库内最多 5 供应商/39 商品/37 进货单/6 销售单/3 生产单/2 盘点单/39 消息）发现**大量优化点** → 先优化后重测；两题按推荐：**Q1=集中修完一次性回归**（模块联动多，零散修易引新回归）、**Q2=重测前清库从空库全量重来**（重置一分钟，结论干净）。用户将开**新会话**带修改清单来。
- **环境重置：** 清库前留保险备份 `~/wms-db-backups/warehouse_management_20260915_posttest.sql`（212K，含首轮测试数据）；库已重置 = 13 账号+6 员工+7 部门、业务全零。坑：WSL 过夜重启清空 /tmp（原清空脚本丢失）且两端 dev 服务已停——脚本重生成 `/tmp/wms-clear.sql`、前后端重启后冒烟过（8080 200 / 登录 token 正常 / 5173 200）。
- **问题 #1 确诊（test-plan 第 5 节已格式化录入）：** 供应商被商品引用后删除**无声失败**——后端守卫正常（「该供应商下仍有关联商品，无法删除」SupplierService:128），病灶前端两层：① request.js 响应拦截器对业务错误（HTTP 200+body code≠200）无全局 toast；② SupplierView `.catch(() => {})` 静默吞异常。**同类静默 catch 模式疑似遍布各页面，修复时全站 grep 排查**——可能是「大量优化点」的共同根因之一。
- **留档提交：** CONTEXT.md/progress/task_plan 修改 + docs/test-plan.md 新增，一笔 docs 提交。
- **下一步：** 新会话收修改清单 → 逐条整理（现象/分类/修法/回归用例）→ 集中修复（bug 优先）→ 接口冒烟+单测 → 空库全量重测 → 《系统使用手册》。

---

## 会话 34 — 2026-09-14

### 阶段 25 全系统测试启动：环境基线（备份/清空/账号清理/空库冒烟 45 全绿）+ 测试计划交付（grill 两轮七题全按推荐，D88）

- **需求（/grill-with-docs）：** 备份数据库 → 只留登录人员数据其余清空 → 交付全系统测试计划（每模块怎么测+为什么）→ 记录测试流程 → 后续产出《系统使用手册》。
- **grill 决策（两轮七题全按推荐，入 D88）：** R1：清空范围=只留账号体系（主数据也清，从建料测起）/ 执行=混合（Claude 接口冒烟+用户文档手测）/ 深度=全面分层（P0 冒烟→P1 负测→P2 边界消息）/ 顺序=业务流串联。R2：范围=除 AI 外全模块（本地 application.properties 无任何 LLM key/base-url 配置，AI 仅记录现象）/ 账号=删 e2e_audit_temp 保留 hxl 系列（后因新事实改为物理清除，见下）/ 测后终态=保留测试数据（备份在手随时可还原）。
- **环境基线执行：** ①备份 `~/wms-db-backups/warehouse_management_20260914_full.sql`（418K，33 表+3 视图+1 存储过程结构校验过；mysqldump 报 PROCESS 权限 warning 为表空间导出，无害）；②清空脚本 `/tmp/wms-clear-20260914.sql`——由 information_schema 动态生成 32 表 TRUNCATE（实际 36 表非 agent 报的 33），保留 sys_user/sys_dept/sys_employee/sys_ip_policy；③空库冒烟 `/tmp/wms-smoke.sh` **45/45 PASS**：13 账号全登录、26 个模块读接口空数据不报错、越权/未登录正确 403/401，另补超管合法 body 建商品 403（ADR-0009 真只读端点级确认）。
- **重要发现——逻辑删除（用户会话中主动提问）：** 用户以为前端删掉的 hxl/hxl2/hxl_sales 实际 `is_deleted=1` 仍在库（MyBatis-Plus 全局逻辑删除，唯一键 uk_username 不含 is_deleted → 同名重建会撞键）。已物理清除 3 行（备份含），最终 13 个种子账号。CONTEXT.md 新增「## 删除」词条（逻辑删除 vs 物理删除两处例外：超管删日志/运维清库）。
- **交付物 `docs/test-plan.md`：** 12 模块（登录/供应商/物料成品/BOM/采购/销售/生产/库存治理/统计/协同/超管/AI）各含「为什么+P0/P1/P2 勾选式用例（步骤到页面路径级，备手册改写）」+ 横切三节（权限矩阵负测/消息生命周期 checklist/已知靶子 T1 退料净额校验缺口·T2 同名撞键·T3 逻辑删除语义）+ 问题记录表+执行记录。
- **坑三枚：** ①TRUNCATE 后 `information_schema.table_rows` 是陈旧缓存（还显示 640/237），核对必须用真实 `COUNT(*)`；②管道退出码陷阱再现——`mysql | grep -v Warning; echo $?` 拿到的是 grep 无匹配行的 exit 1，非 mysql 失败；③`wms_user` 无 CREATE DATABASE 权限 → 备份还原演练不可行，以结构校验（建表/视图/存储过程/INSERT 计数+完成尾注）替代。
- **下一步：** 用户按 `docs/test-plan.md` 手测（顺序 M1→M12），问题记入文档第 5 节由 Claude 修复循环；全部测完后决定数据库终态（D88 默认保留），并从冒烟层用例改写《系统使用手册》。

---

## 会话 33 — 2026-09-14

### 阶段 23 浏览器手测闭环（4 项遗留全过 + 用户顺手走完审核全链路，零代码改动）

- **场景搭建（程序化）：** 起前后端 dev；curl 建场景单 9 `ST260914091503668`（盘点中）——锁定盖 01（账面 47）→负责人仓储管理员(id=5)、底座 01（账面 47）→负责人仓储员工(id=9)。用户手测留的单 3/4 未动（两单均已完成为历史档）。
- **4 项遗留手测全过（用户确认符合预期）：** ① 员工账号「提交审核」按钮可用（D84 修订后解禁）；② 建单对话框负责人分配（默认自己/全部派给我/必选校验）；③ 详情页负责人列 admin 改派下拉；④ 录入人列 + 员工非本人行输入框禁用态。
- **用户超额走完全链路：** admin 录本人行 + 员工录本人行（实盘均 47=账面，差 0）→ 员工提交审核（提交人=仓储员工）→ admin 审核生效 → 已完成。行级录入人分落（仓储管理员/仓储员工，D85 追溯链）实测正确。
- **清理：** 库存 47/47 零变动（差 0 审核无调整）；单 9 已完成终态 API 不可取消（D83），SQL 软删主从表；goods-options 复核商品已释放（occupied=None）且 lastStocktakeTime 派生归零（38/39 恢复「从未盘点」，派生口径无残留）。测试 token 文件已删。
- **小坑：** curl E2E 第一发 401——`application.properties` 配了 `sa-token.token-prefix=Bearer`，请求头须 `Authorization: Bearer <token>`（以往会话均带前缀，本次首发漏带）。
- **下一步候选（仍待拍板）：** ① 阶段 4 余料管理/成品追溯（盘点已落地，剩两项需 grilling 定范围）；② 阶段 16 已知问题（补料单部分到货终态后受 D59 约束无法再补，待单独立项）；③ 异常操作提醒（阶段 22 挂起项，恢复时重议阈值）；④ 移动端适配（2026-09-11 挂起，恢复时续 Round 2，勿从头重问）。

### 阶段 24 补料再补解锁 + 部分到货仍缺料通知（D86–D87 / ADR-0011，阶段 16 已知问题立项修复，TDD 5 例 + curl E2E 全链路）

- **根因（用户拍板「先看一下」→ 只读调研定位）：** D59 幂等守卫实现 `listNonFinalByProductionOrder` 口径=「生产来源 + status≠已驳回(4)」，把终态**已入库(3)** 也算在途。阶段 3.6 起采购申请支持行级部分到货（`arrive_quantity`），而 `confirmReceive` 是整单终态——部分到货入库后生产单仍缺料，守卫按旧口径永远拦截 `createDraft`（死锁）；且 D62「仍缺料沉默」让生产只能靠刷列表发现。现网实测无处于死锁状态的活跃生产单 → 纯行为修复无数据迁移。
- **定案（grill-with-docs 两轮三题，均按推荐）：** ① **D86** 守卫收窄到在途状态（1/2/5，`IN_FLIGHT_STATUSES`），防重复真正闸门是既有「当前无缺料无需补料」实时缺口重算，宽口径守卫冗余且有害；② **D87-a** 详情 VO 透出 `inFlightRequestNo` + 补料弹窗顶部提示+禁用提交（消除"填一屏才被拦"）；③ **D87-b** confirmReceive 重算仍缺 → 发「补料部分到货仍缺料」通知，与「物料已齐套可领料」互斥——标题白名单 `revokeUnreadByBizAndTitles` 撤旧发新（「关联销售单已取消」D73 等同 biz 其他通知不受影响、不堆积）。
- **实现（TDD 垂直切片）：** PurchaseRequestService（常量+查询收窄+`notifyKitAfterReceive` 撤旧发新二选一）、MessageService（两标题常量+白名单撤销+仍缺料发送）、ProductionOrderService.getById 透出在途单号、前端弹窗 alert+禁用。单测 255→260 全绿（守卫 SQL 段只含 1/2/5 不含 3 / InOrder 先撤后发 / 白名单不碰 D73 / getById 透出）。
- **curl E2E 原死锁场景全链路：** 建成品 65+BOM 18（锁定盖 38×1/底座 39×1）→ 生产单 34 → 补料单 26（38×3/39×3）→ 部分到货（38×3/39×1）确认入库终态 → **仍缺料通知内容正确（底座×2）** → **再补 200 成功（D86 解锁点，旧口径此处 400 死锁）** → 在途时再补 400「请勿重复」→ 第二张全到货 → **撤旧仍缺料通知+发齐套通知（白名单机制生产实测）** → stock 47→50 数学正确。sales_admin 权限负测 403 body ✓。
- **清理复原：** 作废单 34（API，触发 D21 撤齐套通知）→ 删 BOM 18（API）→ SQL 软删单 34/补料单 26-27/明细 133-135/入库行 89-91 → 库存复原 47/47 → 删成品 65（API 守卫过）。手测留档盘点单 3/4 未动。
- **坑两枚：** ① login 响应 `data` 是对象 `{token, userInfo}` 非纯 token 串，jq/python 提取须 `['data']['token']`（负测首发 401 虚惊）；② 作废 reason 中文直拼 URL query 被 Tomcat RFC 7230 拒（400 Invalid character），改 `--data-urlencode` 走 form body。
- **留档：** ADR-0011（accepted，修订 D59 实现口径+D62 沉默口径，本体不动）；CONTEXT.md「生产补料」词条重写 + 新增 Avoid「把已入库补料单当在途拦截再补」+「齐套预警」词条改 D87 互斥口径；task_plan D86/D87 入表 + 阶段 24 段落 + 阶段 16 已知问题标记解决。
- **下一步候选（仍待拍板）：** ① 阶段 4 余料管理/成品追溯（需 grilling 定范围）；② 异常操作提醒（挂起重议）；③ 移动端适配（挂起，续 Round 2）。

### 阶段 4 余料管理 grilling 两轮后拍板挂起 + 成品追溯搁置（零代码改动）

- **成品追溯搁置（用户拍板）：** 公司生产成品无非卖出或退回，没有其他流向，追溯无对象——搁置不立项。
- **余料范围反转（用户现场事实）：** 余料 ≠ BOM 剩余（领料严格 BOM×qty 一次性只读，物理剩余不存在）＝ **生产损坏料**；损坏料不进正常仓，现场有破损暂存区。Round 1「完工后补退剩余」方向作废。
- **Round 2 四决策已定（恢复时直接续用）：** ① 范围=损坏登记+暂存/处置闭环+损坏补领通道（仓内补损耗，不绕采购——今天损坏后系统无"再领"通道，线下拿料成账实暗缝）；② 登记绑生产单（上界=该单已领净额服务端兜底，生产成员登记盖章；排除质检报废整单场景）；③ 暂存区=纯台账派生（存量=登记−处置，不建虚拟仓第二份库存真相）；④ 处置=报废丢弃+变卖（不记金额）+研发取用（接协同设计），无回仓复用（损坏料不进正常仓）。
- **派生规则（实现时直接落）：** 可退净额=已领−已退−**已登记损坏**（否则终止自动退料把暂存区损坏件算进可退→库存虚增）；损坏→暂存区纯物理移动无出入库动作。
- **Round 3 已问未答（恢复时续，各带推荐）：** 登记/补领状态窗口（推荐 登记=生产中2+待质检3、补领仅2）；补领通道形态（推荐 新 pick_type=REPICK 第四类型，明细=损坏未补净额带出可改小，仓储既有页确认出库）；处置权限（推荐 生产 admin 收口+留痕）；台账可见性（推荐 生产全员读+超管只读，仓储不挂）；「损坏+仓库也没料」分流（推荐 REPICK 单通道按 min(净额,库存) 带量，罕见场景线下采购，不动 D59 齐套口径——备选 B 缺口公式改消耗感知可顺带修"开工后已发料显示缺口"怪癖，留作未来增强）。
- **用户拍板挂起：** 「太麻烦了，先不做余料了」——阶段 4 三子项终态：盘点 ✅（阶段 23）、成品追溯搁置、余料挂起。
- **顺带发现（独立漏洞，登记 task_plan 候选④）：** 手动退料 `createReturn` 未调 `ensureWithinReturnable`（净额兜底仅终止路径调用）——前端弹窗按已领未退带出，但服务端无兜底，直调 API 可退未领物料/超量退，仓储确认即虚增库存。可独立于余料单独立项小修。
- **纪律：** D1 已修订（决策表）——用户需求以讨论拍板为准，`document/` 不再作为需求依据（已存记忆 wms-requirement-docs-stale）。
- **下一步候选（仍待拍板）：** ① 余料管理（挂起，恢复续 Round 3）；② 异常操作提醒（挂起重议）；③ 移动端适配（挂起，续 Round 2）；④ createReturn 净额兜底小修。
