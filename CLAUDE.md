...
Issues / specs live as local markdown under `.scratch/<feature>/` in this repo (no GitHub Issues). See `docs/agents/issue-tracker.md`.

### Triage labels

Five canonical roles mapped to label strings (`needs-triage`/`needs-info`/`ready-for-agent`/`ready-for-human`/`wontfix`), written as `Status:` lines in issue files. See `docs/agents/triage-labels.md`.

### Domain docs

Single-context layout (root `CONTEXT.md` + `docs/adr/`); not yet present, created lazily by `/domain-modeling`. See `docs/agents/domain.md`.
